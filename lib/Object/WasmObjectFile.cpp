...
Error WasmObjectFile::parseEnumivoABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   enumivo_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("enumivo abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".enumivo_abi") {
     if (Error Err = parseEnumivoABISection(Ctx))
        return Err;
  } else if (Sec.Name == ".enumivo_actions") {
     if (Error Err = parseActionsSection(Ctx))
        return Err;
  } else if (Sec.Name == ".enumivo_notify") {
