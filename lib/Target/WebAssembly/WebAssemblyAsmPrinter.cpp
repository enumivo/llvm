...
void WebAssemblyAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  bool has_abi    = false;
  bool has_enumivo_action = false;
  bool has_enumivo_notify = false;
  for (const auto &F : M) {
     if (F.hasFnAttribute("enumivo_wasm_import"))
        has_import = true;
     if (F.hasFnAttribute("enumivo_wasm_abi"))
        has_abi = true;
     if (F.hasFnAttribute("enumivo_wasm_action"))
        has_enumivo_action = true;
     if (F.hasFnAttribute("enumivo_wasm_notify"))
        has_enumivo_notify = true;
  }
  ...
  if (has_abi) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".enumivo_abi";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("enumivo_wasm_abi")) {
           StringRef abi = F.getFnAttribute("enumivo_wasm_abi").getValueAsString();
  ...
  if (has_enumivo_action) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".enumivo_actions";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("enumivo_wasm_action")) {
           StringRef action_name = F.getFnAttribute("enumivo_wasm_action").getValueAsString();
  ...
  if (has_enumivo_notify) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".enumivo_notify";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("enumivo_wasm_notify")) {
           StringRef name = F.getFnAttribute("enumivo_wasm_notify").getValueAsString();
